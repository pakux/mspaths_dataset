...
def gen_wf(available_inputs=[], report_dir="/home/paulkuntke/mspaths/report"):
    ...
    register =  MapNode(
        RegistrationSynQuick(transform_type="r", precision_type="float"),
    ...
    applymask_flair = MapNode(ApplyMask(output_type='NIFTI_GZ'), iterfield=['in_file'], name='applymask_flair')
    ...
    rename_skullstripped_flair = Node(
        Rename(
            format_string="sub-%(subject)s_ses-%(session)s_desc-brain_FLAIR",
                        keep_ext=True,
                    ),
                    name="rename_skullstripped"
        )
    ...
    registration_wf = coregister_to_mni_wf(skullstripped=True, coreg_masks=True, transform_type='a')
